...
@section('image'){{ config('app.url', 'https://javokhir-media.uz') . '/uploads/' . $post->photo}}@endsection
@section('date'){{ strtotime($post->created_at) }}@endsection
@section('content')
<nav aria-label="breadcrumb m-4">
    <ol class="breadcrumb">
        <li class="breadcrumb-item"><a href="{{ url('/') }}" class="text-decoration-none">Home</a></li>
        <li class="breadcrumb-item"><a href="{{ url('/category/'.$post->category->name) }}" class="text-decoration-none">{{ $post->category->name }}</a></li>
...
<div class="card shadow-sm" data-aos="fade-up" data-aos-duration="1000">
    ...
    <div class="card-body">
        ...
        <p>Share with friends:</p>
         <div class="a2a_kit a2a_kit_size_32 a2a_default_style" data-a2a-url="https://javokhir-media.uz/post/{{ $post->url }}" data-a2a-title="{{ $post->description }}">
...
<div class="mt-4 mb-2">
    @if(!empty($previous))<a class="btn btn-outline-primary leftbtn m-1" href="/{{$previous->url}}"><i class="bi bi-arrow-left" aria-hidden="true"></i> Previous page</a>@endif
    @if(!empty($next))<a class="btn btn-outline-primary rightbtn m-1" href="/{{$next->url}}">Next page <i class="bi bi-arrow-right" aria-hidden="true"></i> </a>@endif
</div>
@endsection
